...
DWORD CMeshObject::AddVertex( UINT hash, CVertex* pVertex )
{
	bool bFound = false;
	DWORD index = 0;
/*
...
void CMeshObject::LoadGeometryFromOBJFile(const TCHAR *szFileName)
{
...
void CMeshObject::LoadMaterialsFromMTLFile( const TCHAR *szFileName )
{	
	// File input
	TCHAR strCommand[256] = {0};
#ifdef UNICODE
...
/*
void CMeshObject::CreateObjectFromFile(ID3D10Device *pDevice, const char *szFileName)
{
	if(pDevice)
	{
		CleanUp();
...
unsigned long CMeshObject::GetNumSubsets()
{
	return 0;//m_numAttribTableEntries; 
}

CMaterial * CMeshObject::GetSubsetMaterial(unsigned long iSubset)
{
	return NULL;//m_materials[m_pAttribTable[iSubset].AttribId];
}

/*
const CAABB &CMeshObject::GetAABB()
{
	return aabb;
}

void CMeshObject::GetAABB( CVector3 &vecLBN, CVector3 &vecRTF)
{
...
CObject *CMeshObject::CollisionDetect( const CVector3 &vecTranslate)
{
	/*
	CVector3 axes[3];
...
/*
CObject *CMeshObject::CollisionDetect( CObject *pObj, const COBB &obbA)
{
...
/*
CObject *CMeshObject::CollisionDetect( CObject *pObj, const CVector3 &vecLBN, const CVector3 &vecRTF)
{
